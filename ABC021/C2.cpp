...
public:
	...
	Dijekstra() : V(0) { }
	Dijekstra(size_t v) :
		V(v),E(v,vector<pr>(0)),dist(v,2147483647),trace(v,2147483647),createdDag(false),memo(v,VI(v,loINF)){}
...
*/
	long dfs(long step,long node) {
		...
		REP(i,E[node].size()) {
			if (dist[E[node][i].first] - dist[node] == E[node][i].second){
