...
public:
	...
	void resetDijekstra(void) {
		dist = vector<long>(V,2147483647);
		trace = vector<long>(V,2147483647);
	}
};
...
int main(void) {
	...
	REP(i,M) {
		...
		else dj.add_edge_undirected(u-1,v-1,l);
	}
	...
	REP(i,next) {
		//dump(b[i].first)
		dj.dijekstra(b[i].first);
		FOR(j,i+1,next) {
			tmp = dj.result_query(b[j].first);
