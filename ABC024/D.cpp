...
int main(void) {
	...
	LL A,B,C;
	cin >> A >> B >> C;
	LL c_up = ((B%MOD) * ((C - A)%MOD))%MOD;
	LL c_down = it.mod_inv((((A-B)%MOD)*(C%MOD))%MOD+(((A%MOD)*(B%MOD))%MOD));
	LL c = (c_up * c_down)%MOD;
	dump(c_up)
	dump(c_down)
	if (c < 0) c += MOD;
	LL r_up = ((B - A)%MOD * (C%MOD))%MOD;
	LL r_down = it.mod_inv(((((A-C)%MOD) * (B%MOD))%MOD) + (((A%MOD)*(C%MOD))%MOD));
	dump(r_up)
	dump(r_down)
	LL r = (r_up * r_down)%MOD;
	if (r < 0) r += MOD;
	cout << r << " " << c << endl;
	return 0;
}
