...
int main(void) {
	...
	VI A(N);
	REP(i,N) cin >> A[i];
	Dijekstra dj(N),dj_r(N);
	REP(i,M){
		cin >> a >> b >> c;
		dj.add_edge_directed(a-1,b-1,c);
		dj_r.add_edge_directed(b-1,a-1,c);
	}
	dj.dijekstra(0);
	dj_r.dijekstra(0);
	long ans = 0;
	long val_go,val_back;
	long val;
	REP(i,N) {
		val_go = dj.result_query(i);
		val_back = dj_r.result_query(i);
		//dump(val_go);
		//dump(val_back);
		val = (T - val_go - val_back)*A[i];
		if (ans < val) ans = val;
	}
